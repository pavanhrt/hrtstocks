...
// prints, or writes UPSTOX_ACCESS_TOKEN to
...
//
// Usage:
//   DATABASE_URL=postgres://... UPSTOX_ACCESS_TOKEN=... \
//     node services/pipeline/scripts/collect-upstox-sectors.mjs [--out FILE] [--delay-ms N]
//
...
import { createUpstoxClient, UpstoxApiError } from "../src/fundamentals/providers/upstox-client.js";
import { classifyUpstoxSector, UPSTOX_SECTOR_TAXONOMY_VERSION } from "../src/fundamentals/providers/upstox-sector-taxonomy.js";
...
async function main() {
  ...
  const { data: instruments, error } = await ops.select(db, `select id, symbol, isin from instruments
        ...
        order by symbol asc`, [false], "many");
