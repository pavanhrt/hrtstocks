...
test("only an existing-object conflict is safe immutable reuse", () => {
  ...
  assert.equal(isExistingChartObjectError({ code: 412, message: "At least one of the pre-conditions you specified did not hold." }), true);
  assert.equal(isExistingChartObjectError({ code: 500, message: "storage unavailable" }), false);
