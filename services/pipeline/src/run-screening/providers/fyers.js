...
// replacing the NSE public-endpoint scraper that cloud egress IPs got
// 403-blocked from. Free with a Fyers trading account; see
// scripts/fyers-get-token.mjs for how the access token is produced.
//
// Auth model: FYERS_APP_ID (an OAuth client_id analog -- useless without a
// token) and FYERS_ACCESS_TOKEN come from the environment via
// ./fyers-credentials.js (Secret Manager in Cloud Run). The token is a real
// secret that expires daily (SEBI requirement) and is refreshed MANUALLY each
// trading day -- this project deliberately does not automate Fyers login (that
// would require storing a trading PIN + TOTP secret, a much bigger credential
// surface). An expired token stops the run with a clear FyersAuthError.
//
 ...
 */
async function requestHistory(url, symbol, db) {
  return serializeFyersRequest(() => requestHistoryOnce(url, symbol, db));
}

async function requestHistoryOnce(url, symbol, db) {
  let res, body;
 ...
 * @param {Date} fromDate
 * @param {Date} toDate
 * @param {import("../../db/client.js").Db} [db] when
 ...
 *   have a database handle handy; production callers must pass it.
 */
export async function fetchOHLCVRange(instrumentId, symbol, fromDate, toDate, db = null) {
  ...
  const body = await requestHistory(url, symbol, db);
 ...
 */
export async function fetchOHLCV(instrumentId, symbol, days, db = null, asOf = new Date()) {
  const to = new Date(asOf);
  const from = new Date(to.getTime() - days * 24 * 60 * 60 * 1000);
  return fetchOHLCVRange(instrumentId, symbol, from, to, db);
}
