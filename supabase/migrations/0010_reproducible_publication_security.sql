...
-- These new evidence tables are service-role write-only. Viewer-facing
-- run-scoped rows remain hidden until the owning run is completed.
alter table run_universe_sources enable row level security;
alter table run_universe_instruments enable row level security;
alter table analysis_bars enable row level security;
alter table pipeline_persistence_errors enable row level security;
alter table run_publication_manifests enable row level security;

create policy run_universe_sources_read on run_universe_sources
  for select to authenticated using (
    exists (
      select 1 from screening_runs r where r.id = run_universe_sources.run_id
        and (r.publication_state = 'published' or public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin'))
    )
  );
create policy run_universe_instruments_read on run_universe_instruments
  for select to authenticated using (
    exists (
      select 1 from screening_runs r where r.id = run_universe_instruments.run_id
        and (r.publication_state = 'published' or public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin'))
    )
  );
create policy analysis_bars_read on analysis_bars
  for select to authenticated using (public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin'));
create policy pipeline_persistence_errors_read on pipeline_persistence_errors
  for select to authenticated using (public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin'));
create policy run_publication_manifests_read on run_publication_manifests
  for select to authenticated using (
    exists (
      select 1 from screening_runs r where r.id = run_publication_manifests.run_id
        and (r.publication_state = 'published' or public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin'))
    )
  );

revoke all on table run_universe_sources, run_universe_instruments, analysis_bars,
  pipeline_persistence_errors, run_publication_manifests from anon, authenticated;
grant select on table run_universe_sources, run_universe_instruments, analysis_bars,
  pipeline_persistence_errors, run_publication_manifests to authenticated;
...
revoke execute on function expire_stale_screening_runs(timestamptz) from public, anon, authenticated;
revoke execute on function try_acquire_rate_limit_slot(text, timestamptz, integer) from public, anon, authenticated;
revoke execute on function claim_next_pipeline_batch(uuid) from public, anon, authenticated;
revoke execute on function reset_stale_pipeline_batches(uuid, integer, integer) from public, anon, authenticated;
grant execute on function publish_screening_run(uuid) to service_role;
grant execute on function expire_stale_screening_runs(timestamptz) to service_role;
grant execute on function try_acquire_rate_limit_slot(text, timestamptz, integer) to service_role;
grant execute on function claim_next_pipeline_batch(uuid) to service_role;
grant execute on function reset_stale_pipeline_batches(uuid, integer, integer) to service_role;

-- =============================================================================
-- Authorization hardening (explicitly approved after risk disclosure).
--
-- Every policy touched below already existed and was already select-only and
-- role-checked in its USING clause (see 0002_rls.sql, 0005_direction_analysis.sql,
-- 0006_durable_pipeline_and_swing_analysis.sql, 0007_provider_rate_limit_buckets.sql);
-- none of them granted anonymous access to anything a role check didn't already
-- block. This section is defense-in-depth, not a behavior fix for a live leak:
--
--   1. Every affected policy is scoped explicitly `TO authenticated` (ALTER POLICY
--      changes only the policy's role list / USING clause -- it does not drop and
--      recreate the policy, so no window exists where the table is unprotected).
--      Previously these policies applied to PUBLIC and relied entirely on their
--      USING clause (auth.role() = 'authenticated' or current_role_name() checks)
--      to exclude anon; scoping the policy itself to authenticated removes that
--      reliance and matches the same explicit-role pattern already used for every
--      table created earlier in this same migration.
--   2. Every run-scoped Viewer-visible read policy that gated on
--      `screening_runs.status = 'completed'` now gates on
--      `screening_runs.publication_state = 'published'` instead. status='completed'
--      could previously be set by legacy/partial code paths without the
--      transactional validation publish_screening_run() now performs; Viewer-role
--      access to run-scoped result tables now requires the same publication
--      guarantee the frontend's data helpers already require (see
--      CONTINUATION.md workstream 6). Researcher/strategy_admin/system_admin
--      access is unchanged -- they still see in-flight and unpublished runs for
--      operational purposes.
--   3. Default anon/authenticated table privileges (INSERT/UPDATE/DELETE/
--      TRUNCATE/REFERENCES/TRIGGER, and anon's SELECT) are revoked from every
--      computed/pipeline table below and replaced with SELECT-only for
--      authenticated. RLS with no write policy already blocked writes from these
--      roles (see 0002_rls.sql's header comment: all writes come from the
--      service-role Edge Function, which bypasses RLS); this removes the
--      underlying grant so a future RLS policy mistake or a table with RLS
--      accidentally disabled cannot silently reopen write access.
--   4. Internal pipeline RPCs are already service_role-only as of this same
--      migration's earlier section (publish_screening_run, expire_stale_screening_runs,
--      try_acquire_rate_limit_slot, claim_next_pipeline_batch,
--      reset_stale_pipeline_batches); no further RPC exists that needs restricting.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1 + 2. Explicit `TO authenticated` role scoping, and viewer publication gate
-- ---------------------------------------------------------------------------

-- profiles / strategy-parameter registry / universe reference data: role scope only.
alter policy profiles_self_or_admin_select on profiles to authenticated;
alter policy profiles_admin_update on profiles to authenticated;
alter policy strategy_versions_read on strategy_versions to authenticated;
alter policy rule_definitions_read on rule_definitions to authenticated;
alter policy parameter_versions_read on parameter_versions to authenticated;
alter policy instruments_read on instruments to authenticated;
alter policy index_memberships_read on index_memberships to authenticated;

-- Raw/researcher-only data: already role-gated by current_role_name(); role scope only.
alter policy market_bars_raw_read on market_bars_raw to authenticated;
alter policy market_bars_adjusted_read on market_bars_adjusted to authenticated;
alter policy corporate_actions_read on corporate_actions to authenticated;
alter policy data_quality_results_read on data_quality_results to authenticated;
alter policy derivative_contracts_read on derivative_contracts to authenticated;
alter policy derivative_snapshots_read on derivative_snapshots to authenticated;
alter policy pipeline_audit_log_read on pipeline_audit_log to authenticated;
alter policy screening_run_leases_read on screening_run_leases to authenticated;
alter policy pipeline_batches_read on pipeline_batches to authenticated;
alter policy provider_rate_limit_buckets_read on provider_rate_limit_buckets to authenticated;

-- instrument_direction: latest-state presentation data, not run-scoped -- role
-- scope only, no publication_state gate applies (see comment 2 above).
alter policy instrument_direction_read on instrument_direction to authenticated;
alter policy direction_charts_read on storage.objects to authenticated;

-- Run-scoped Viewer-visible results: role scope AND publication_state gate.
alter policy screening_runs_read on screening_runs
  to authenticated
  using (
    publication_state = 'published' or public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin')
  );

alter policy instrument_run_results_read on instrument_run_results
  to authenticated
  using (
    exists (
...
alter policy rule_traces_read on rule_traces
  to authenticated
  using (
    exists (
      select 1 from screening_runs r
...
alter policy rankings_read on rankings
  to authenticated
  using (
    exists (
      select 1 from screening_runs r
...
alter policy coverage_reconciliation_read on coverage_reconciliation
  to authenticated
  using (
    exists (
      select 1 from screening_runs r
...
alter policy instrument_direction_runs_read on instrument_direction_runs
  to authenticated
  using (
    exists (
      select 1 from screening_runs r
...
alter policy direction_pivots_read on direction_pivots
  to authenticated
  using (
    exists (
      select 1 from screening_runs r
...
alter policy elliott_hypotheses_read on elliott_hypotheses
  to authenticated
  using (
    exists (
      select 1 from screening_runs r
...
alter policy pattern_detections_read on pattern_detections
  to authenticated
  using (
    exists (
      select 1 from screening_runs r
...
alter policy instrument_alignment_read on instrument_alignment
  to authenticated
  using (
    exists (
      select 1 from screening_runs r
...
alter policy swing_analysis_results_read on swing_analysis_results
  to authenticated
  using (
    exists (
      select 1 from screening_runs r
...
alter policy swing_analysis_rule_traces_read on swing_analysis_rule_traces
  to authenticated
  using (
    exists (
...
-- ---------------------------------------------------------------------------
-- 3. Revoke default anon/authenticated write privileges on every computed/
-- pipeline table; re-grant only the authenticated SELECT each already has via
-- RLS. `profiles` and `bootstrap_admin_emails` are intentionally excluded:
-- profiles_admin_update legitimately requires an UPDATE grant for the
-- authenticated role (system_admin connects as Postgres role `authenticated`,
-- distinguished only by profiles.role, and RLS already restricts which rows/
-- callers that UPDATE succeeds for); bootstrap_admin_emails already has RLS
-- enabled with zero policies, which blocks all access unconditionally.
-- ---------------------------------------------------------------------------
