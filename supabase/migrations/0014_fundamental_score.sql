...
-- mirroring supabase/functions/fundamentals/binding.js's own
...
-- supabase/functions/fundamentals/binding.js's resolveFundamentalBinding()
...
-- ---------------------------------------------------------------------------
-- REVISED 2026-09-16 (correction 5) -- both bind functions are operational
-- RPCs, never callable from a browser session: a plain Viewer/Researcher
-- reads bindings only through buy_setup_analysis_ledger's own RLS-gated
-- view, never by resolving one directly. Restricting EXECUTE the same way
-- 0011's publish_buy_setup_enrichment is restricted -- explicit REVOKE from
-- PUBLIC/anon/authenticated, GRANT only to service_role.
-- ---------------------------------------------------------------------------

revoke execute on function bind_fundamental_scores_for_refresh(bigint) from public, anon, authenticated;
grant execute on function bind_fundamental_scores_for_refresh(bigint) to service_role;
revoke execute on function bind_fundamental_scores_for_run(uuid) from public, anon, authenticated;
grant execute on function bind_fundamental_scores_for_run(uuid) to service_role;

-- ---------------------------------------------------------------------------
-- RLS: SELECT-only to authenticated, gated by the score's own refresh
-- manifest state -- identical pattern to buy_setup_manifests (0011).
-- Ingestion/scoring writes are server-role-only (no INSERT/UPDATE/DELETE
-- grant to anon/authenticated anywhere below); browser clients can never
-- write an authoritative score.
-- ---------------------------------------------------------------------------

alter table fundamental_source_snapshots enable row level security;
alter table fundamental_score_versions enable row level security;
alter table fundamental_refresh_manifests enable row level security;
alter table fundamental_score_results enable row level security;
alter table fundamental_score_components enable row level security;
alter table buy_setup_fundamental_score_bindings enable row level security;

-- Raw snapshots and spec versions are operational/source-of-truth data, not
-- published research evidence -- Researcher+ only, same treatment as
-- buy_setup_pipeline_batches/buy_setup_persistence_errors.
create policy fundamental_source_snapshots_read on fundamental_source_snapshots
  for select to authenticated using (public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin'));
create policy fundamental_score_versions_read on fundamental_score_versions
  for select to authenticated using (public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin'));
create policy fundamental_refresh_manifests_read on fundamental_refresh_manifests
  for select to authenticated using (public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin'));

create policy fundamental_score_results_read on fundamental_score_results
  for select to authenticated using (
    refresh_manifest_id is null
      and public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin')
    or exists (
      select 1 from fundamental_refresh_manifests m
      where m.id = fundamental_score_results.refresh_manifest_id
        and (m.refresh_state = 'published' or public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin'))
    )
  );
create policy fundamental_score_components_read on fundamental_score_components
  for select to authenticated using (
    exists (
      select 1 from fundamental_score_results r
      left join fundamental_refresh_manifests m on m.id = r.refresh_manifest_id
      where r.id = fundamental_score_components.score_result_id
        and (
          (m.id is null and public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin'))
          or m.refresh_state = 'published'
          or public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin')
        )
    )
  );

-- A binding is only ever visible if the result it points to is (RLS also
-- enforces this on fundamental_score_results itself, but buy_setup_analysis_ledger
-- is security_invoker, so the CALLING user's own grant on THIS table is what
-- actually gates the join -- it must repeat the same condition, not merely
-- rely on the join target's own policy filtering the visible rows).
create policy buy_setup_fundamental_score_bindings_read on buy_setup_fundamental_score_bindings
  for select to authenticated using (
    exists (
      select 1 from fundamental_score_results r
      left join fundamental_refresh_manifests m on m.id = r.refresh_manifest_id
      where r.id = buy_setup_fundamental_score_bindings.fundamental_score_result_id
        and (m.refresh_state = 'published' or public.current_role_name() in ('researcher', 'strategy_admin', 'system_admin'))
    )
  );

revoke all on table
  fundamental_source_snapshots, fundamental_score_versions, fundamental_refresh_manifests,
  fundamental_score_results, fundamental_score_components, buy_setup_fundamental_score_bindings
