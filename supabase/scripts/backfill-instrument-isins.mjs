...
// access token, only SUPABASE_SERVICE_ROLE_KEY to read/write `instruments`.
//
...
//
// Usage:
//   SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node supabase/scripts/backfill-instrument-isins.mjs [--apply] [--out FILE]
...
async function main() {
  ...
  for (const row of toUpdate) {
    const { error: updateError } = await supabase.from("instruments").update({ isin: row.isin }).eq("id", row.id).is("isin", null);
