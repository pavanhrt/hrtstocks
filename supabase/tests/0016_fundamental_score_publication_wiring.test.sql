...
select ok(
  not has_function_privilege('anon', 'public.publish_fundamental_refresh(bigint)', 'EXECUTE')
  and not has_function_privilege('authenticated', 'public.publish_fundamental_refresh(bigint)', 'EXECUTE')
  and has_function_privilege('service_role', 'public.publish_fundamental_refresh(bigint)', 'EXECUTE'),
  'publish_fundamental_refresh EXECUTE is restricted to service_role only'
);
...
select ok(
  not has_function_privilege('authenticated', 'public.publish_screening_run(uuid)', 'EXECUTE')
  and has_function_privilege('service_role', 'public.publish_screening_run(uuid)', 'EXECUTE'),
  'publish_screening_run keeps its original service_role-only EXECUTE restriction after being replaced'
);
